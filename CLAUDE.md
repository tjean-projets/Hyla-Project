...
### Tier 3 — infra & perf (juillet 2026)
- **PWA installable** (`vite.config.ts` + `src/main.tsx`) : vite-plugin-pwa v1.3 avec autoUpdate + manifest complet (theme_color #3b82f6, background_color #0b0e18, display standalone, start_url /dashboard, icons any + maskable). Ancien `public/sw.js` + `public/manifest.json` custom SUPPRIMÉS (le plugin les régénère à `dist/sw.js` + `dist/manifest.webmanifest`). Runtime cache : Supabase REST NetworkFirst 24h (fresh > stale, timeout 5s), Storage CacheFirst 7j, tuiles OSM CacheFirst 30j. SPA navigateFallback → index.html. Résultat : app installable sur écran d'accueil iOS/Android + pages consultées restent accessibles offline en lecture.
- **Widget Rythme du mois** (`pages/Dashboard.tsx > MonthlyRhythmWidget`) : décompose `user_settings.monthly_sales_target` en cible par jour + par semaine + reste à faire. Barre de progression avec marqueur "où on devrait être" par rapport au temps écoulé. Statut auto (En avance / Pile / X ventes de retard / Objectif atteint). Astuce actionnable si rythme requis > 1.5× cible.
- **Code-splitting agressif** (`vite.config.ts > build.rollupOptions.output.manualChunks`) : bundle `index.js` passé de **604 → 137 kB** (-77%). 10 vendor chunks séparés : vendor-react, vendor-supabase, vendor-router, vendor-query, vendor-radix, vendor-charts (recharts+d3), vendor-xlsx, vendor-icons, vendor-maps, vendor-dnd, vendor-markdown. Les gros vendor (charts 421 kB, xlsx 424 kB) sont maintenant lazy-loaded uniquement par les pages qui les utilisent.
- **Types Supabase régénérés** : `src/integrations/supabase/types.ts` régénéré (749 → 1849 lignes) avec `supabase gen types typescript --project-id wtkhzxgizyajtwtgtgkg`. Couvre les tables/colonnes récentes : campaigns, task_columns, message_templates, assigned_to_member_id, member_objectives.follow_up_*, deal_status.en_financement. Les `(supabase as any)` restants peuvent être nettoyés progressivement.
